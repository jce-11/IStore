...
public class QueryServlet extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
         // get parameters from search form and check they contain values 
         ...
         if (!hasCategoryParam && !hasSearchParam) {  // No params present
            out.println("<h3> select a category or enter a search term!</h3>");
            out.println("<p><a href='start'>Back</a></p>");
         } else {
            conn = DriverManager.getConnection(databaseURL, username, password);
            stmt = conn.createStatement();
            // build  SQL command based on from the entered search terms
